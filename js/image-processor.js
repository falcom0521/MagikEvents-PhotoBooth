...
class ImageProcessor {
    ...
    try {
      ...
      if (frameDataUrl) {
        ...
        return new Promise((resolve, reject) => {
          ...
          img.onerror = () => {
            console.error(
              "Error loading frame from Firebase, creating default placeholder",
            );
        ...
        console.log(
          "No active frame in Firebase, creating default placeholder",
        );
  ...
  createDefaultFrame() {
    return new Promise((resolve) => {
      ...
      const qualitySettings = window.cameraManager?.getQualitySettings() || {
        finalWidth: 1200,
        finalHeight: 1800,
      };
  ...
  updatePreview() {
    ...
    const maxOffsetX = Math.max(
      (previewWidth - containerW) / 2,
      containerW / 2,
    );
    const maxOffsetY = Math.max(
      (previewHeight - containerH) / 2,
      containerH / 2,
    );
    ...
    if (this.targetImage && targetImg) {
      ...
      const displayTargetImage = () => {
        try {
          ...
          if (
            this.targetImage.complete &&
            this.targetImage.width > 0 &&
            this.targetImage.height > 0
          ) {
            ...
            if (this.targetImage.complete) {
              ...
              this.targetImage.addEventListener("load", handleLoad, {
                once: true,
              });
        ...
        (e) => {
          ...
          if (e.ctrlKey || e.metaKey) {
            ...
            const newZoom = Math.max(
              10,
              Math.min(500, this.zoomLevel - scrollAmount * 2),
            );
  ...
  async processImages() {
    if (!this.backgroundImage) {
      throw new Error(
        "Missing background image. Please capture a photo first.",
      );
    }
    ...
    console.log(
      `Target image state: complete=${this.targetImage?.complete}, width=${this.targetImage?.width}, height=${this.targetImage?.height}`,
    );
    console.log(
      `Background image state: complete=${this.backgroundImage.complete}, width=${this.backgroundImage.width}, height=${this.backgroundImage.height}`,
    );

    const targetReady = await this.waitForImageLoad(this.targetImage, 5000);
    const backgroundReady = await this.waitForImageLoad(
    ...
    if (!targetReady || !this.targetImage.width || !this.targetImage.height) {
      throw new Error(
        "Frame image failed to load. Please wait a moment and try again, or refresh the page.",
      );
    }
    ...
    if (
      ...
      throw new Error(
        "Background image failed to load. Please capture the photo again.",
      );
    }
    ...
    console.log(
      `Target image: ${this.targetImage.width}x${this.targetImage.height}`,
    );
    console.log(
      `Background image: ${this.backgroundImage.width}x${this.backgroundImage.height}`,
    );
    ...
    try {
      ...
      const maxOffsetX = Math.max(
        (scaledWidth - canvas.width) / 2,
        canvas.width / 2,
      );
      const maxOffsetY = Math.max(
        (scaledHeight - canvas.height) / 2,
        canvas.height / 2,
      );
